...
class AsciiFilter {
	constructor(renderer, args) {
		...
		this.charset = args.charset || ' .\'`^",:;Il!i~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$'
...
class CanvAscii {
    ...
    updateRotation() {
        let x = Math.map(this.mouse.y, 0, this.height, .5, -.5)
        let y = Math.map(this.mouse.x, 0, this.width, -.5, .5)
        this.mesh.rotation.x += (x - this.mesh.rotation.x) * .05
        this.mesh.rotation.y += (y - this.mesh.rotation.y) * .05
    }

}

const _asciiText = new CanvAscii("smelted")
